...
holidays = ["01-Jan-24", "15-Jan-24", "19-Feb-24", "29-Mar-24", "10-Apr-24",
            "27-May-24", "04-Jul-24", "02-Sep-24", "28-Nov-24", "25-Dec-24"]

dirpath = "/root/stockdata/"

# Load data from the JSON file
with open('/root/stockTicker/stock-options-update/usa_data.json', 'r') as f:
    data = json.load(f)
...
def track_stocks(list_stocks):
    ...
    dow_file_name_full = os.path.join(dirpath, "DOW_CALL_" + today_date)
...
def calculate_profit(headers, cookies):
    ...
    last_price = get_current_price("^DJI", 0)
    perc_change, change = get_change("^DJI")
    statement += f"<b>Dow Jones:{round(last_price)}, change:{change}({perc_change}%)</b>\n\n"
...
def main():
    ...
    if now.strftime("%d-%b-%y") in holidays or now.weekday() > 4:
        return
    else:
