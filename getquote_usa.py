...
target_profit = [[1000, -200], [1600, -400], [1400, -300]]
#tracking list ex: [['POWERGRID.NS', 228, 0], ['SBIN.NS', 460, 1]]  #0 for going up, 1 for going down
target_stocks = [
                    ['AAON', 68.71, 1], ['TBBK', 38.92, 1], ['FOR', 32.5, 1],
                    ['AAON', 80, 0], ['TBBK', 45.1, 0], ['FOR', 37.6, 0]
                ]
...
def calculate_profit(headers, cookies):
    ...
    statement += "<b>Dow Jones:{}, change:{}({}%)</b>\n\n".format(round(last_price), change, perc_change)
    if not len(holdings):
        return
...
def main():
    ...
    cookies = '' #Not using currently, hence set to empty string
